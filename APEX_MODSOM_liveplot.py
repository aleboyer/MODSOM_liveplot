...
class ByteSourceThread(threading.Thread):
    def __init__(
        ...
        mode_command_delay_s: float = 0.05,
        mode_command: Optional[bytes] = b"efe.mode 0\r\n",
        start_command: Optional[bytes] = b"efe.start\r\n",
        stop_command: Optional[bytes] = b"efe.stop\r\n",
        ...
        self.mode_command_delay_s = mode_command_delay_s
        ...
        try:
            if self.mode == "serial" and self.start_command:
                try:
                    ...
                    self._send_serial_cmd(self.mode_command)
                    if self.mode_command_delay_s > 0:
                        time.sleep(self.mode_command_delay_s)
    ...
    def _parse_efe4_record(self, rec: Record, inst: EFE4Data):
        ...
        if not inst.file_mode:
            ...
            for i in range(n_samples):
                ...
                if len(sample_ts_bytes) != 8 or len(chan_bytes) != 12:
                    ts = ts[:i]
                    t1 = t1[:i]
                    # t2 = t2[:i]
                    s1 = s1[:i]
                ...
                c_s1 = bytes3_to_signed_int(chan_bytes[3:6])
                c_a3 = bytes3_to_signed_int(chan_bytes[6:9])
                c_a2 = bytes3_to_signed_int(chan_bytes[9:12])
                # c_a1 = bytes3_to_signed_int(chan_bytes[12:15])
                # c_t2 = bytes3_to_signed_int(chan_bytes[15:18])
                # c_s2 = bytes3_to_signed_int(chan_bytes[18:21])
...
def main():
    ...
    ap.add_argument("--baud", "-b", type=int, default=230400, help="Serial baudrate")
    ...
    if args.file:
        ...
        source_thread = ByteSourceThread(
            ...
            stop_event=stop_event,
            mode_command=b"efe.mode 0\r\n",
            start_command=b"efe.start\r\n",
            stop_command=b"efe.stop\r\n",
        )
        source_thread.start()
